        ...
        if (TF()):
            if buttons["text"] == " " and bclick == True:
                ...
                tkinter.messagebox.showinfo(
                    "Tic-Tac-Toe", "Button already Clicked!")
        else:
            # disableButton()
            tk.destroy()
            tkinter.messagebox.showinfo(
                "Tic-Tac-Toe", "please fill player name before starting the Game")
            # normalButton()
...
def main():
    ...
    button2 = Button(g, bg='lightblue', height=2, width=20,
                     text='Multiplayer', fg='red', bd=3, command=multi_player)
    # button2.place(bordermode=INSIDE)
    button2.grid(row=3, column=1)

    button3 = Button(k, bg='lightblue', height=2, width=20,
                     text='single player', fg='red', bd=3, command=single_player)
    # button2.place(bordermode=INSIDE)
